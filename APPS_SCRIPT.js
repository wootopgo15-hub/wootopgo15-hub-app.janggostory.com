  ...
  try {
    ...
    try {
      ...
      if (text.toLowerCase().includes('error')) {
        if (text.includes('The number of columns in the range must be at least 1')) {
          throw new Error(`스프레드시트 빈 시트 오류입니다. 연동된 스프레드시트 탭(예: REPORT 또는 USER)의 첫 번째 줄(A1, B1 등)에 열 제목(헤더)을 입력해주세요.`);
        }
    ...
    if (resultData && resultData.result === 'error') {
      if (resultData.message && resultData.message.includes('The number of columns in the range must be at least 1')) {
        throw new Error(`스프레드시트 빈 시트 오류입니다. 연동된 스프레드시트 탭의 첫 번째 줄에 열 제목(헤더)을 입력해주세요.`);
      }
